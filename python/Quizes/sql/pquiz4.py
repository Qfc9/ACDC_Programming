...
def main():
    ...
    query = "SELECT winner_id, DATE_FORMAT(start, '%c-%d-%Y'), "\
            ...
            "FROM Fight "\
            "WHERE combatant_one = %s AND combatant_two = %s;"
    result = standards.getSQLData(query, [c1, c2])
    ...
    query = "SELECT c1.name, c2.name, winner.name "\
            ...
            "WHERE c1.id = %s AND c2.id = %s AND winner.id = %s;"
    result = standards.getSQLData(query, [c1, c2, winner])
